...
async function displayMovies(responseData){
    ...
    for (let i = 0; i < dataArray.length; i++){
        if (dataArray[i].backdrop_path !=null){
            ...
            let moreData = await fetch(`https://api.themoviedb.org/3/movie/${dataArray[i].id}?api_key=${API_KEY}&language=en-US`)
            ...
            `
            <div class = "movie-card">
                <img src = "${IMG_PREFIX + dataArray[i].poster_path}" class = "movie-poster" alt = "movie image"/>
                <div class = "movie-info">
                    <h3 class = "movie-title"> ${dataArray[i].original_title} </h3>
                    <p class = "movie-votes"> ${dataArray[i].vote_average}/10 </p>
